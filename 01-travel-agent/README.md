...
想要做旅行计划而没有头绪时，现代年轻人通常会在小红书进行搜索。而想把它变成自己能直接用的行程表，还需要多个步骤去规划、手动记录以下几个问题:哪天去哪、吃什么、住哪。这个 Agent 把这件事自动化了。你把旅游行程(文字、截图或分享链接)输入,可以得到一份结构化、可校验的每日行程 JSON。

![Agent 四阶段架构](./assets/cover_architecture.png)

## 它是怎么工作的

它不是"把文本喂给大模型,让它吐个 JSON"那么简单。那种做法会有多个风险:地名被翻译两遍导致漂移、模型编造原文里没有的城市、遇到无关内容也硬要输出。按照课程理解，我把它拆成了四步流程:

1. **感知(Perceive)**:先看懂你给的是文字、截图还是链接,并判断这到底是不是一篇行程类内容。不是的话,在花钱调大模型之前就早退。
2. **检索(Retrieve)**:从一个北海道旅行知识库里找相关背景,帮它读懂那些口语化的地名。（出于成本控制考虑mvp版本只做了北海道地点的知识库）
3. **行动(Act)**:把"洞爷湖万世阁大浴场"这种带描述的说法对齐到标准地点。拿不准的给人确认，实现HITL，支持人为修改。
4. **校验(Validate)**:输出前自查。天数合不合理?有没有编出原文没有的城市?不靠谱的结果直接拦下来。

## 我在这个项目里做的

- **不让模型乱翻译地名。** 中文地名一律保留原文,靠一个专门的地点映射器(PlaceMapper)做唯一真相来源。线上系统里"翻译两次导致语义漂移"是很常见的问题,我从架构上把它堵死了。
- **适当节省 token** 遇到无关或含糊的输入,在调用大模型之前就拒掉;检索层完全不做向量化,省掉 embedding 的开销（这有个最大的原因是我们拿到的学校资源api并不支持embedding）。
- **人机协作只在关键处触发。** 交通、购物、明显是中转的条目自动跳过,只把"真正需要人判断"的抛给用户确认,尽量少打扰。
- **把"负责任 AI"做成能跑的测试,而不是口号。** 内置了 5 条幻觉护栏、提示注入攻击测试(有人故意输入"忽略以上所有指令"),还做了有无示例的对照实验来量化效果。

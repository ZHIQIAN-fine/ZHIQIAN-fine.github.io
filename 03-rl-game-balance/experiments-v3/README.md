...
| **v3b** 数值修正 | 按「每费伤害」重算三张废卡：404·死灵协议 9→13、代码污染 5→7、亵渎协议 8→15 | [`v3b_card_stats.cards.diff`](patches/) |
| v3ab | 两者叠加 | 两个 patch 同时打 |

v3a 的分档顺序不是随手排的，依据是「这张卡是否需要成型牌组才敢要」。例如格式化记忆的效果是从牌组里移除卡，在只有 10 张基础牌时它移除的只能是匕首或护盾，是纯负面；只有牌组被掉落撑厚、混进废牌之后它才变成解毒剂。所以它必须晚放。
...
先看可信度：**v3b 的效应是真的**，两个种子只差 0.5pt（71.6 / 71.1），比基线自身的 4.2pt 波动小一个量级。v3a 的失败也在两个种子上方向一致。这就是双种子的用处——它给出了一条噪声底线，让我能判断多大的变化才算变化。
...
（一个诚实的边界：v3b 同时改了三张卡、局长也同时变短，严格说「是抽卡机会而非数值」这个归因没有完全分离变量。要干净地证明它，需要一个控制局长的实验，我没有跑。）
...
| `models/` | 12 次训练（4 组 × 3 种子：42 / 7 / 123）的权重 |

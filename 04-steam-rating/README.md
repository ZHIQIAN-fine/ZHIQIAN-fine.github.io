...
特征工程是这个项目里我花时间最多的地方,一共构造了 61 维特征(清洗后表宽 62 列,其中 1 列是目标变量 `positive_ratio`):
...
模型是一个 Keras MLP:61 → 128 →(Dropout 0.3)→ 64 →(Dropout 0.2)→ 32 → 1,输出层用 Sigmoid 把预测值卡在 0~1,共 18,305 个参数;Adam(lr=0.001),MSE 损失,batch 32,训 30 轮。验证损失稳定收敛在 0.023 附近,没有过拟合。
